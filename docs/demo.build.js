...
var require_utils = __commonJS({
  "node_modules/@noble/hashes/utils.js"(exports) {
    ...
    var rotr3 = (word, shift2) => word << 32 - shift2 | word >>> shift2;
    exports.rotr = rotr3;
...
function weierstrassPoints(opts) {
  ...
  const wnaf = wNAF(Point3, CURVE2.endo ? Math.ceil(_bits / 2) : _bits);
  return {
    CURVE: CURVE2,
...
function decodeASL(publicKey, geobits = SANE_DEFAULT) {
  if (typeof publicKey === "string")
    publicKey = hexToBytes2(publicKey);
...
var Geohash = class {
  ...
  static encode(lat, lon, precision) {
    if (typeof precision == "undefined") {
      for (let p = 1; p <= 12; p++) {
        const hash3 = Geohash.encode(lat, lon, p);
        const posn = Geohash.decode(hash3);
...
var JacobianPoint = class {
  ...
  static fromAffine(p) {
    ...
    return new JacobianPoint(p.x, p.y, _1n6);
  }
  ...
  double() {
    ...
    const C = mod2(B * B);
    const x1b = X1 + B;
    const D = mod2(_2n5 * (mod2(x1b * x1b) - A - C));
    const E = mod2(_3n3 * A);
    const F = mod2(E * E);
    const X3 = mod2(F - _2n5 * D);
    const Y3 = mod2(E * (D - X3) - _8n2 * C);
    const Z3 = mod2(_2n5 * Y1 * Z1);
...
JacobianPoint.BASE = new JacobianPoint(CURVE.Gx, CURVE.Gy, _1n6);
JacobianPoint.ZERO = new JacobianPoint(_0n6, _1n6, _0n6);
...
var Point2 = class {
  ...
  static fromHex(hex2) {
    const bytes3 = ensureBytes2(hex2);
    const len = bytes3.length;
  ...
  multiplyAndAddUnsafe(Q, a, b) {
    ...
    const aP = a === _0n6 || a === _1n6 || this !== Point2.BASE ? P.multiplyUnsafe(a) : P.multiply(a);
...
var Signature = class {
  ...
  static fromDER(hex2) {
    ...
    const { r, s } = parseDERSignature(arr ? hex2 : hexToBytes3(hex2));
...
function isWithinCurveOrder(num) {
  return _0n6 < num && num < CURVE.n;
}
function isValidFieldElement(num) {
  return _0n6 < num && num < CURVE.P;
}
function kmdToSig(kBytes, m, d, lowS = true) {
  ...
  let recovery = (q.x === sig.r ? 0 : 2) | Number(q.y & _1n6);
...
var InternalSchnorrSignature = class {
  ...
  initNonce(d, t0h) {
    return numTo32b2(d ^ bytesToNumber(t0h));
  }
  finalizeNonce(k0h) {
  ...
  finalizeSig(R, k, e, d) {
    return new SchnorrSignature(R.x, mod2(k + e * d, CURVE.n)).toRawBytes();
  }
  error() {
    throw new Error("sign: Invalid signature produced");
  }
  async calc() {
    ...
    if (!await schnorrVerify2(sig, m, px))
      this.error();
    return sig;
  }
  calcSync() {
...
async function schnorrSign2(msg, privKey, auxRand) {
...
function finalizeSchnorrVerify(r, P, s, e) {
  const R = Point2.BASE.multiplyAndAddUnsafe(P, normalizePrivateKey(s), mod2(-e, CURVE.n));
  if (!R || !R.hasEvenY() || R.x !== r)
    return false;
  return true;
}
async function schnorrVerify2(signature, message, publicKey) {
  try {
    const { r, s, m, P } = initSchnorrVerify(signature, message, publicKey);
    const e = schnorrChallengeFinalize(await utils.taggedHash(TAGS.challenge, numTo32b2(r), P.toRawX(), m));
...
function schnorrVerifySync(signature, message, publicKey) {
  try {
    const { r, s, m, P } = initSchnorrVerify(signature, message, publicKey);
    const e = schnorrChallengeFinalize(utils.taggedHashSync(TAGS.challenge, numTo32b2(r), P.toRawX(), m));
    return finalizeSchnorrVerify(r, P, s, e);
  } catch (error) {
...
var utils = {
  ...
  hashToPrivateKey: (hash3) => {
    ...
    const num = mod2(bytesToNumber(hash3), CURVE.n - _1n6) + _1n6;
    return numTo32b2(num);
  },
...
var base322 = chain(radix2(5), alphabet("ABCDEFGHIJKLMNOPQRSTUVWXYZ234567"), padding(5), join(""));
...
utils.hmacSha256Sync = (key, ...msgs) => hmac2(sha2562, key, utils.concatBytes(...msgs));
...
var HDKey = class {
  ...
  sign(hash3) {
    ...
    bytes2(hash3, 32);
    return signSync(hash3, this.privKey, {
      canonical: true,
      der: false
    });
  }
  verify(hash3, signature) {
    ...
    return verify(sig, hash3, this.publicKey);
  }
  wipePrivateData() {
    this.privKey = void 0;
...
function getPublicKey2(privateKey) {
  return utils.bytesToHex(schnorr2.getPublicKey(privateKey));
}
var utils_exports2 = {};
...
function getPow(id) {
  return getLeadingZeroBits(utils.hexToBytes(id));
}
...
function createDelegation(privateKey, parameters) {
  ...
  let sig = utils.bytesToHex(
    schnorr2.signSync(sighash, privateKey)
  );
  return {
...
function getDelegator(event) {
  ...
  if (!schnorr2.verifySync(sig, sighash, pubkey))
    return null;
  return pubkey;
}
var nip27_exports = {};
...
utils.hmacSha256Sync = (key, ...msgs) => hmac2(sha2562, key, utils.concatBytes(...msgs));
utils.sha256Sync = (...msgs) => sha2562(utils.concatBytes(...msgs));
...
function boot() {
  ...
  document.getElementById("music-switch").addEventListener("change", (ev) => {
    if (!player)
      return;
    if (!ev.target.value)
      player.resume();
...
var ModPlayer = class {
  ...
  needsResync = true;
  t0 = 0;
  /* Sync point in audio ctx */
  s0 = 0;
  /* Sync point in xm ctx */
  amp = 1;
  clip = false;
  xmActions = [];
  constructor(RATE = 48e3, AUDIO_BUFFER_LENGTH = 4096) {
    this.RATE = RATE;
    ...
    this.LATENCY_COMP = RATE * (this.audioContext.outputLatency | this.audioContext.baseLatency | 0.25) - RATE / 60;
  ...
  runXCTX(action) {
    ...
    if (this.xmActions.length > 1)
      return;
    while (this.xmActions.length)
      this.xmActions.shift()();
  }
  ...
  async loadModule(buffer) {
    ...
    if (!this.moduleContext)
      throw new Error(`Loading failed, moduleCtx: ${this.moduleContext}`);
  }
  fillBuffer(buffer) {
    ...
    for (let off = 0; off < this.AUDIO_BUFFER_LENGTH; off += this.XM_BUFFER_LENGTH) {
      ...
      for (let j = 0; j < this.XM_BUFFER_LENGTH; ++j) {
        ...
        if (!this.clip && (l[j] < -1 || l[j] > 1 || r[j] < -1 || r[j] > 1)) {
          this.clip = true;
        }
      }
    }
